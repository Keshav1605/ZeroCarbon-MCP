...
class TouchTexture {
  ...
  addTouch(x: number, y: number) {
    ...
    if (last) {
      const dx = last.x - x;
      const dy = last.y - y;
      force = Math.min((dx * dx + dy * dy) * 30000 + 0.25, 1.0);
    }
...
export function InteractiveParticles({
  ...
  useEffect(() => {
    ...
    const applyScale = () => {
      ...
      // Align the left edge of the mesh to the left edge of the screen
      const meshWidth = imgWidth * scale;
      const viewportWidth = fovHeight * camera.aspect;
      container3D.position.x = -viewportWidth / 2 + meshWidth / 2;
    };
